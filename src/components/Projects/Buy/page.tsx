...
export const BuyPage: React.FC<BuyPageProps> = ({
  ...
  const handleAdditionalEthChange = (
    event: ChangeEvent<HTMLInputElement>,
    index: number
  ) => {
    const newWallets = [...initialWallets];
    ...
    <div>
      <div className="flex justify-between mt-5 mb-5">
        <div className="flex gap-4">
          <p className="flex items-center gap-2 mb-2 font-medium text-[#71717A] text-sm">
            Selected:{" "}
        ...
        <div className="flex gap-4">
          <Button
            className="bg-[#09090B] border-none font-normal text-[#F57C00] text-sm"
        ...
        <Table className="border-[#18181B] border-[1px] rounded-md text-center">
          ...
          <TableBody>
            {initialWallets?.map((wallet, index) => (
              <TableRow
                key={wallet.address}
                className={`hover:bg-inherit py-0 border-none ${index % 2 === 1 ? "bg-[#18181B]" : ""}`}
              >

...
export const SimulateSellTx = ({
    projectData,selectedWallets,onSelectionChange
}: SimulateTxProps) => {
  const {address} = useAccount();  
  console.log(selectedWallets);
  const [isSimulating, setIsSimulating]  = useState(false);
  const [sellWallets, setSellWallets] = useState<Wallet[]>(selectedWallets);
    ...
    try {
      ...
      for(let i=0; i<simulationData.length; i++){
        if(simulationData[i].transaction.status === false){
          toast("Simulation Failed: " + simulationData[i].transaction.error_message + 'For: ' + simulationData[i].transaction.from )
          setIsSimulating(false);
          return;
        }
      }
  
      for(let i=0, j=(sellWallets?.length! * 2); i<sellWallets?.length!; i++, j++){
        const assetChanges = simulationData[j].transaction.transaction_info.asset_changes
  ...
  const getEthTransferTxSequence = () => {
    ...
    for(let i=0; i<sellWallets.length; i++){
        const ethValue = parseFloat(formatUnits(BigInt('250000'), 9));
  ...
  const getApproveTx = () => {
    try {
        ...
        const tx: Transaction = {
            ...
            to: tokenAddress,
            input: tokenInterface.encodeFunctionData('approve', [uniswapV2RouterAddress,parseUnits(sellWallets[i].tokenToSell!,18)]) 
        }
  ...
  const getSellTx = () => {
    try {
        ...
        const tx: Transaction = {
            ...
            input: uniswapRouterInterface.encodeFunctionData('swapExactTokensForETH', [parseUnits(sellWallets[i].tokenToSell!,18),'0',[tokenAddress,wethAddress],sellWallets[i].address,deadline]) 
        }

...
export const Sell: React.FC<SellPageProps> = ({
  wallets,
  balances,
  onSelectionChange,
}) => {
  ...
  const handleSelectOne = (
    ...
    if (event.target.checked) {
      ...
      onSelectionChange(
        initialWallets.filter((wallet) =>
          updatedSelection.includes(wallet.address)
        )
      );
    } else {
      const updatedSelection = selectedWallet.filter(
  ...
  const isSelected = (walletAddress: string) =>
    selectedWallet.includes(walletAddress);
  ...
  const downloadCSV = (data: Wallet[], selectedWallets: string[]) => {
    const selectedData = data.filter((wallet) =>
    ...
    <div>
      <div className="flex justify-between mt-5 mb-5">
        <div className="gap-4 flex">
          <p className="text-[#71717A] text-sm font-medium mb-2 flex gap-2 items-center">
            Selected:{" "}
            <span className="text-white">{selectedWallet.length}</span>
          </p>
        </div>
        <div className="gap-4 flex">
          <Button
            className="bg-[#09090B] border-none text-[#F57C00] text-sm font-normal"
      ...
      <Table className="border-[1px] border-[#18181B] rounded-md">
        ...
        <TableBody>
          {initialWallets?.map((wallet, index) => (
            <TableRow
              key={wallet.address}
              className={`hover:bg-inherit py-0 border-none text-center ${index % 2 === 1 ? "bg-[#18181B]" : ""}`}
            >
              ...
              <TableCell className="text-[#A1A1AA] text-[12px]">
                {index + 1}
              </TableCell>
              <TableCell className="py-0">
                <div className="text-[#71717A] flex gap-1 items-center text-[12px]">
                  ...
                  <Image
                    ...
                    alt="Copy"
                    onClick={() =>
                      handlePublicKeyCopy(wallet.address, wallet.privateKey)
                    }
                  />
                </div>
              </TableCell>
              <TableCell className="py-0">
                <div className="text-[#F57C00] flex gap-1 items-center justify-center text-[12px]">
                  <Image src={"/Vector.svg"} width={15} height={15} alt="ETH" />
              ...
              <TableCell className="py-0 text-[#A1A1AA] text-[12px]">
                {wallet?.ethEstimated}
              </TableCell>
              <TableCell className=" py-0">

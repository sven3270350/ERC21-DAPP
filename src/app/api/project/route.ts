  ...
  try {
    for (let i = 0; i < projectIds.length; i++) {
      ...
      if (!project.beneficiaryDetails) {
        ...
        const encryptedWallets = project.beneficiaryDetails.map(
          (value: any) => {
...
export async function PUT(request: NextRequest) {
  ...
  try {
    for (let i = 0; i < projectIds.length; i++) {
      ...
      const bundleEncryptPK = encrypt(project.bundleWallet.privateKey);
      project.bundleWallet.privateKey = bundleEncryptPK;
      project.beneficiaryDetails = encryptedWallets;

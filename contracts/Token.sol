...
contract Token is ERC20, Ownable {
    ...
    function setDevBuyTax(uint256 _tax) external onlyOwner {
        devBuyTax = _tax;
    }
    ...
    function setDevSellTax(uint256 _tax) external onlyOwner {
        devSellTax = _tax;
    }

    function setMarketingBuyTax(uint256 _tax) external onlyOwner {
        marketingBuyTax = _tax;
    }

    function setMarketingSellTax(uint256 _tax) external onlyOwner {
        marketingSellTax = _tax;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (from == swapPair && devBuyTax + marketingBuyTax > 0) {
                ...
                uint256 marketingTax = (amount * marketingBuyTax) / 100;

                transferAmount = amount - devTax - marketingTax;
                super._transfer(from, devWallet, devTax);
                super._transfer(from, marketingWallet, marketingTax);
            } else if (to == swapPair && devSellTax + marketingSellTax > 0) {
                uint256 devTax = (amount * devSellTax) / 100;
                uint256 marketingTax = (amount * marketingSellTax) / 100;
